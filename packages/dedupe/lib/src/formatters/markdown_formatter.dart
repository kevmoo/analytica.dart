...
class MarkdownFormatter {
  ...
  String format(DedupeReport report) {
    ...
    _writeSummary(buffer, report.summary);

    if (includeFileTable && report.fileMetrics.isNotEmpty) {
      _writeFileTable(buffer, report.fileMetrics);
    }

    if (includeClusters) {
      _writeClusters(buffer, report);
    }

    return buffer.toString().trimRight();
  }

  static void _writeSummary(StringBuffer buffer, DedupeSummary summary) {
  ...
  static void _writeFileTable(
    StringBuffer buffer,
    List<FileDuplicationMetric> metrics,
  ) {
    final duplicateFiles = metrics.where((f) => f.duplicateLines > 0).toList()
      ..sort(
        (a, b) => b.duplicationPercentage.compareTo(a.duplicationPercentage),
      );
  ...
  static void _writeSingleCluster(
    ...
    final diffBadge = cluster.intersectsDiff
        ? (cluster.isNewlyIntroduced ? ' `[NEW IN DIFF]`' : ' `[IN DIFF]`')
        : '';
