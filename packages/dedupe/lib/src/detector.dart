...
const int _primeBase = 31337;
const int _hashMask = 0x7FFFFFFFFFFFFFFF;
...
class CloneDetector {
  ...
  List<DuplicateCluster> detect(
    ...
    final nonSubsumedPairs = filterSubsumedPairs(rawPairs);
    final dsu = SpanDsu();
    ...
    final clusters = createClustersFromSpanGroups(clusterMap, fileSequences);
...
bool _verifyMinHashCandidateTokens(
  AstCandidateUnit c1,
  AstCandidateUnit c2,
  List<TokenSequence> fileSequences,
...
Map<int, List<_TokenLocation>> _buildKgramIndex(
  List<TokenSequence> fileSequences,
  int k,
  int basePow,
) {
...
(int, int, int, int) _computeLocationPairKey(
  _TokenLocation loc1,
  _TokenLocation loc2,
) {
...
(int, int, int, int) _computeSpanPairKey(TokenSpan span1, TokenSpan span2) {
