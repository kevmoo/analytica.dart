...
/// Command-line interface orchestration for pkg:lower_bound.
class LowerBoundCli {
  ...
  static void _printUsage(ArgParser parser, StringSink out) {
    out.writeln(
  ...
  static List<String>? _resolvePackagePaths(
  ...
  static List<String>? _resolveExplicitPaths(
    List<String> rawPaths,
    StringSink err,
  ) {
    final paths = <String>[];
  ...
  static List<String>? _resolveCwdWorkspace(String cwd, StringSink err) {
  ...
  static Future<int> _runValidationSuite(
    ArgResults results,
  ...
  static void _renderResults({
    ...
    if (commentOutput != null && commentOutput.isNotEmpty) {
      final commentReport = LowerBoundReporter.buildMarkdownReport(
        results,
        maxCommentRows: maxCommentRows,
      );
      try {
    ...
    switch (format) {
      case 'json':
        LowerBoundReporter.renderJson(results, sink);
        break;
      case 'github':
        LowerBoundReporter.renderGitHub(
          results,
          sink,
      ...
      case 'text':
      default:
        LowerBoundReporter.renderText(results, sink);
        break;
    }
  }
}
