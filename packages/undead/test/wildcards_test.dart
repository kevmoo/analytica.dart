...
'''),
          ...
          check(reportDefault.pureUndeadFound).equals(3);
...
'''),
          ...
          check(reportDefault.pureUndeadFound).equals(1); // UnusedStub
          check(
            reportDefault.testedUndeadFound,
          ...
          check(reportCustom.pureUndeadFound).equals(1);
          check(reportCustom.testedUndeadFound).equals(1);

          final pureUndead = reportCustom.undead.firstWhere(
            (z) => z.classification == UndeadClassification.pureUndead,
          );
          check(pureUndead.name).equals('UnusedStub');

          final testedUndead = reportCustom.undead.firstWhere(
            (z) => z.classification == UndeadClassification.testedUndead,
          );
          check(testedUndead.name).equals('RegularDeadClass');
        },
      );
    });
...
'''),
          ...
          check(summary['testedUndead']).equals(0);

          final undead = json['undead'] as List<dynamic>;
          check(undead.length).equals(1);
          final firstUndead = undead.first as Map<String, dynamic>;
          check(firstUndead['name']).equals('real_dead');
        },
      );
...
'''),
          ...
          check(undead.length).equals(1);
          check(
            (undead.first as Map<String, dynamic>)['name'],
          ).equals('actual_dead');
        },
      );
    });
