...
ArgParser buildArgParser() {
  return ArgParser()
    ...
    ..addHelpFlag()
    ..addVersionFlag(help: 'Print undead version.');
}
...
class UndeadCliRunner {
  ...
  Future<int> run(List<String> args) async {
    ...
    if (results.flag('help')) {
      outSink.writeln(
        'undead - Reachability and dead declaration analysis for Dart '
        'packages.',
      );
      outSink.writeln();
      outSink.writeln('Usage: undead [options] [target_path]');
      outSink.writeln();
    ...
    if (results.flag('version')) {
      outSink.writeln('undead version: $undeadVersion');
      return ExitCode.success.code;
    }
    ...
    try {
      ...
      if (failOnUndead && report.undead.isNotEmpty) {
        return 1;
      }
